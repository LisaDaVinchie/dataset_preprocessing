...
notify_telegram() {
    ...
    -d chat_id=${chat_id} \
    -d text="Your modis download slurm job (Job ID: $SLURM_JOB_ID) has completed with status: $status"
}
...
$PYTHON src/download_modis_slurm.sh || { echo "MODIS download failed"; exit 1; }
