...
notify_telegram() {
    ...
    -d chat_id=${chat_id} \
    -d text="Your dataset_preprocessing download slurm job (Job ID: $SLURM_JOB_ID) has completed with status: $status"
}
...
make download || { echo "Cutting failed"; exit 1; }
