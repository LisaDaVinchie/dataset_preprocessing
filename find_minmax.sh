...
notify_telegram() {
    ...
    -d chat_id=${chat_id} \
    -d text="Your dataset_preprocessing, minmax-finding slurm job (Job ID: $SLURM_JOB_ID) has completed with status: $status"
}
...
make minmax || { echo "Training failed"; exit 1; }
