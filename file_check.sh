...
notify_telegram() {
    ...
    -d chat_id=${chat_id} \
    -d text="Your dataset_preprocessing file checking job (Job ID: $SLURM_JOB_ID) has completed with status: $status"
}
...
$PYTHON src/file_check.py || { echo "File check failed"; exit 1; }
