...
#   random starting point for the cutted images, chosen in a range
...
class CutImages:
    ...
    def _get_available_days(self, path_list: list) -> dict:
        ...
        for path in path_list:
            ...
            available_days[key] = [day for day in range(1, num_days + 1)]
            if (year, month) == oldest_month:
                available_days[key] = available_days[key][self.surrounding_days:]
            if (year, month) == latest_month:
                available_days[key] = available_days[key][:-self.surrounding_days]
            
        return available_days
    
    def get_data_paths_to_dataset_idx_dict(self, path_list_init: list[Path], files_dir: Path) -> tuple[dict, th.Tensor]:
        ...
        max_row = row_range[1] - self.final_nrows
        max_col = col_range[1] - self.final_ncols
        if max_row < row_range[0] or max_col < col_range[0]:
            raise ValueError(f"Invalid row or column range: {row_range}, {col_range}. The maximum row and column must be greater than the minimum row and column.")
        # print("n rows:", self.final_nrows, "n cols:", self.final_ncols, flush=True)
        # print(f"Row range: {row_range}, Col range: {col_range}", flush=True)
        # print(f"Max row: {max_row}, Max col: {max_col}", flush=True)
        
        # Initialize the dictionary
        final_dict = {}
        
        mean_img_per_day = max(1, self.n_images // len(path_list_init))
        
        
        path_list_init = sorted(path_list_init)
        
        max_image_nans = int(0.2 * self.final_nrows * self.final_ncols)
        self.max_mask_pixels = int(0.3 * self.mask_class.square_nrows * self.mask_class.square_nrows)
        self.max_trials = 1000
        
        # Iterate over the paths
        i = 0
        k = 0
        # selected_days_dict = {path.stem: [] for path in path_list}  # Dictionary to keep track of the selected days for each path
        selected_days_dict = self._get_available_days(path_list_init)
        
        while i < self.n_images:
            path_list = list(selected_days_dict.keys())
            
            if len(path_list) == 0:
                raise ValueError("No available paths left to process. Please check the input paths and the number of images requested.")
            ...
            # if path == path_list_init[-1]:
            #     max_day = original_nan_mask.shape[0] - self.surrounding_days
            #     print(f"day range for path {path.stem} is {min_day}-{max_day}", flush=True)
            # available_days = [day for day in range(min_day, max_day + 1) if day not in selected_days_dict[path.stem]]
            ...
            while n_images_this_month > 0:
                if i >= self.n_images:
                    ...
                    break
                if len(selected_days_dict[path]) <= 0:
                    print(f"No available days for path {path.stem}. Skipping this month.", flush=True)
                    break
                
                day = random.choice(selected_days_dict[path])
                
                selected_days_dict[path].remove(day)  # Remove the day from the available days
                if len(selected_days_dict[path]) == 0:
                    del selected_days_dict[path]
                    break  # If there are no more available days for this path, break the loop
                
                # Get the index of the point as (x, y) coordinates and the valid cutted image
                n_nans = th.inf
                trials = 0
                while n_nans > max_image_nans and trials < self.max_trials:
                    point = [random.randint(row_range[0], max_row), random.randint(col_range[0], max_col)]
                    cutted_mask = original_nan_mask[day - 1, point[0]:point[0] + self.final_nrows, point[1]:point[1] + self.final_ncols]
                    n_nans = th.sum(~cutted_mask)
                    trials += 1
                if trials >= self.max_trials:
                    print(f"\tCould not find a valid point for day {day} in file {path.stem} after {trials} trials. Skipping this day.", flush=True)
                    continue  # Skip this day if the point is not valid after max_trials
                    
                
                mask = self.mask_class.mask()
                
                n_nans_in_mask = th.inf
                trials = 0
                while n_nans_in_mask > self.max_mask_pixels and trials < self.max_trials:
                    mask = self.mask_class.mask()
                ...
                # get the position of the point in the dataset
                # as i = batch index, j = channel index
                year_month_day_str = f"{year_month_str}_{day:02d}"
                neighboring_days_list = self._get_days_list(year_month_day_str)
                
                for (j, year_month_day_str) in enumerate(neighboring_days_list):
    ...
    def cut(self, files_to_days_and_points_dict: dict) -> th.Tensor:
